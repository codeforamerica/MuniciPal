class CosaCouncilDistrict < ActiveRecord::Base
  ...
  def self.inDistrict? lat, long
    ...
    @spec_area = CosaCouncilDistrict.where("ST_Contains(geom, ST_SetSRID(ST_Transform(ST_SetSRID(ST_MakePoint(?, ?), ?), ?), ?))",
                                              long,
                                              lat,
                                              COORD_SYS_REF,
                                              COORD_SYS_AREA,
  ...
  def self.getDistrict lat, long
    ...
    @area_in_geojson = CosaCouncilDistrict.find_by_sql("select district, name, sqmiles, shape_leng, shape_area, ST_AsGeoJSON(ST_Transform(ST_SetSRID(geom, #{COORD_SYS_AREA}), #{COORD_SYS_REF}))
                                                        from cosa_council_districts 
